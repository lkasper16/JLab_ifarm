...
public:
  ...
  GemInputHandler();//offline file analysis
  GemInputHandler(const char* ipAddr, int tcpPort, const char* etFile, size_t size = 1048576) throw(GemException);//online monitoring
  ~GemInputHandler();
  ...
  int ProcessMultiEventsFromFile(TString filename, unsigned int minEntry, unsigned int maxEntry, TH1F** ADCHist, TH1F** HitHist, TH1F** ClusterHist, TH1F** ClusterInfoHist, TH2F** pos2DHist, TH2F** chargeSharingHist, TH1F** chargeRatioHist, TH2F** timeBinPosHist, TH2F** adcTimeBinPosHist);
