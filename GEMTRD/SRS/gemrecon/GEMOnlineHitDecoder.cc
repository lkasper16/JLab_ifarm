...
//======================================================
GEMOnlineHitDecoder::GEMOnlineHitDecoder(Int_t iEntry, Int_t nbOfTS, Int_t startTS, Int_t stopTS, Int_t zeroSupCut, Int_t comModeCut, TString isPeakOrSum, TString centralOrAllStrips, Float_t minADCs, Int_t minClustSize, Int_t maxClustSize, Int_t maxClustMult) {
...
void GEMOnlineHitDecoder::ProcessEvent( map<int, map<int, vector<int> > > srsSingleEvent , GEMPedestal * ped ) {
  ...
  for(it=mSrsSingleEvent.begin(); it!=mSrsSingleEvent.end(); ++it)  {
	...
	TString detectorType = fMapping->GetDetectorTypeFromDetector(fMapping->GetDetectorFromAPVIDMap()[fAPVID]); 
...
void GEMOnlineHitDecoder::APVEventDecoder() {
  ...
  std::transform (rawDataZS.begin(), rawDataZS.end(), rawDataZS.begin(), std::bind2nd(std::divides<Float_t>(), (Float_t) fNbOfTimeSamples)) ;
  ...
  for(Int_t timebin = 0; timebin < fNbOfTimeSamples; timebin++) {
	...
	GEMHit * hit = new GEMHit(hitID, fAPVID, chNo, 0, fIsHitMaxOrTotalADCs, fNbOfTimeSamples, fStopTimeSamples, fStartTimeSamples) ;
	...
	if(!fListOfHitsClean[hitID]) {
	  GEMHit * hit = new GEMHit(hitID, fAPVID, chNo, fZeroSupCut, fIsHitMaxOrTotalADCs, fNbOfTimeSamples, fStopTimeSamples, fStartTimeSamples) ;
...
void GEMOnlineHitDecoder::APVEventSplitChannelsDecoder() {
  ...
  for(Int_t timebin = 0; timebin < fNbOfTimeSamples; timebin++) {
    ...
    // PERFORM APV25 PEDESTAL OFFSET CORRECTION  FOR A GIVEN TIME BIN
    std::transform (rawDataTS_1stSet.begin(), rawDataTS_1stSet.end(), fPedestalOffsets_1stSet.begin(), rawDataTS_1stSet.begin(), std::minus<int>());

    // COMPUTE THRESOLD (= 20 PEDESTAL NOISE) FOR COMMON MODE DATA SELECTION 
    threshold_1stSet.clear();
    threshold_1stSet.resize(rawDataTS_1stSet.size());
    std::transform (fPedestalNoises_1stSet.begin(), fPedestalNoises_1stSet.end(), threshold_1stSet.begin(), std::bind2nd(std::multiplies<Float_t>(), 50.0));
    ...
    std::transform(fPedestalNoises_2ndSet.begin(), fPedestalNoises_2ndSet.end(), threshold_2ndSet.begin(), std::bind2nd(std::multiplies<Float_t>(), 20.0)); 
  ...
  // ADC AVERAGE OVER ALL TIME BINS USE AS THE TEST CRITERIA FOR ZERO SUPPRESSION
  std::transform (rawDataZS_1stSet.begin(), rawDataZS_1stSet.end(), rawDataZS_1stSet.begin(), std::bind2nd(std::divides<Float_t>(), (Float_t)fNbOfTimeSamples)) ;
  std::transform (rawDataZS_2ndSet.begin(), rawDataZS_2ndSet.end(), rawDataZS_2ndSet.begin(), std::bind2nd(std::divides<Float_t>(), (Float_t)fNbOfTimeSamples)) ;
...
void GEMOnlineHitDecoder::ComputeClusters() {
  ...
  for (hitsFromPlane_itr = fListOfHitsCleanFromPlane.begin(); hitsFromPlane_itr != fListOfHitsCleanFromPlane.end(); ++hitsFromPlane_itr) {
	...
	clustersMap[clusterNo] = new GEMCluster(fEventNb, clusterNo, fMinClusterSize, fMaxClusterSize, fIsCentralOrAllStripsADCs, fMinADCvalue) ;
    ...
    cout << "fListOfClustersCleanFromPlane " << plane << " size = " << fListOfClustersCleanFromPlane[plane].size() << endl;
    hitsFromPlane.clear() ;
    clustersMap.clear() ;
  }
}
...
void GEMOnlineHitDecoder::GetTimeBinClusterHit(TString plane, TH2F* the2dHist) {
  ...
  for(cluster_it=listOfClusters.begin(); cluster_it!=listOfClusters.end(); ++cluster_it) {
	...
	the2dHist -> Fill( (25 * (*timebin_it).first), (* cluster_it)->GetHit(i)->GetStripNo(), (*timebin_it).second );
      }
    }
    nbOfAnalysedClusters++ ;
  }
...
//======================================================
void GEMOnlineHitDecoder::FillADCvsDriftTimeAndPositionForLargestCluster(TString plane, TH2F* the2dHist, TH2F* adcHist) {
...
//======================================================
void GEMOnlineHitDecoder::FillClusterHistos(TString plane, TH1F* posHist, TH1F* adcUniHist, TH1F* adcDistHist, TH1F* clusterSizeHist, TH1F* clusterMultHist) {
...
//=========================================================================================================================
void GEMOnlineHitDecoder::FillPos2D(TH2F* posHist, TH2F* adcUniHist, TH2F* timeSampleHist, Float_t xpos, Float_t ypos, Float_t adc, Float_t timing) {
...
//====================================================================================
void GEMOnlineHitDecoder::Fill2DClusterHistos(TString detector, TH2F* pos2DHist,  TH2F* adc2DHist, TH2F* timeSampleHist, TH2F* chargeSharingHist, TH1F* chargeRatioHist) {
  // printf(" Enter  GEMOnlineHitDecoder::Fill2DClusterHistos() \n") ;

  list<GEMCluster*> listOfClustersX = fListOfClustersCleanFromPlane[fMapping->GetPlaneListFromDetector(detector).front()];
  list<GEMCluster*> listOfClustersY = fListOfClustersCleanFromPlane[fMapping->GetPlaneListFromDetector(detector).back()];
  ...
  for(clusterX_itr = listOfClustersX.begin(); clusterX_itr != listOfClustersX.end(); ++clusterX_itr ) clusterListX->Add(* clusterX_itr) ;
  for(clusterY_itr = listOfClustersY.begin(); clusterY_itr != listOfClustersY.end(); ++clusterY_itr ) clusterListY->Add(* clusterY_itr) ;
