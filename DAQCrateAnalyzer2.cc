...
int main(int argc, char *argv[]) {  
 ...
 to_sergey_norad2.open("to_sergey_dedx_norad2.dat",ios::out);
  ...
  int Slot=SLOT;
	 ...
	 TCanvas *myc;
	 myc = NULL; //new TCanvas("myc", "Event", 600, 600);

	 //=========================================================================
	 //                 SRS 
	 //=========================================================================
	 cout << " Create  GemView  " << endl;
	 gemView = new GemView(RunNumber);
  ...
  sprintf(InputFile,"hd_rawdata_%06d.evio",RunNumber);
    ...
    fdcFeTree->Branch( "gemx", &gemx, "gemx/F" );
    fdcFeTree->Branch( "gemy", &gemy, "gemy/F" );
...
string str1(ss.str());
  ...
  //sprintf(InputFile,"hd_rawdata_%s_00%d.evio",str1,OK-1);
  sprintf(InputFile,"hd_rawdata_%06d_%03d.evio",RunNumber,OK-1);
  sprintf(fnam,"%s/%s",DataDir,InputFile);
  ...
  EvOchan.open();
   ...
   TRIGGER_MASK_GT=0;
	    ...
	    //if (gemclust.size()!=1) continue; // SRS 
...
*/
    ...
    gemx=-1000;
    gemy=-1000;
    for (int ic=0; ic<gemclust.size(); ic++) {
       gemx=gemclust[ic].x;
       gemy=gemclust[ic].y;
    }
         fdcFeTree->Fill();
        ...
        if(DISPLAY){
          ...
          int OKw2=0;
          int OKw=0;
         if(abs(wchmax-(15.-gemx*0.04))<2.){
          cout<<" W_X and GEM_x = "<<wchmax<<" "<<15.-gemx*0.04<<" "<<wchmax-(15.-gemx*0.04)<<endl;
             ...
             for (Int_t ch=0;ch<24;ch++){ 
              if(wAmax[ch]>thresh){
                Ywaxis[ch][i-smp1] = ADCSamples[0][slot][ch+ch0][i]-ADCPedestal[0][slot][ch+ch0];
                  if(Ywaxis[ch][i-smp1]>thresh/3.)wct_plot->Fill((float)(ch+wchoffset)*10.,(float)(i-smp1)/10.,Ywaxis[ch][i-smp1]);
                  if(Ywaxis[ch][i-smp1]>thresh)OKw=1;
               }
             } //end ch loop
// u
             slot=sl_c1u1; ch0=ch_c1u1;
             for (Int_t ch=0;ch<24;ch++){ 
              if(uAmax[ch]>thresh/8.){
             ...
             for (Int_t ch=0;ch<48;ch++){ 
              if(dAmax[ch]>thresh){
                ...
                Ydaxis[ch][i-smp1] = ADCSamples[0][slot][k][i]-ADCPedestal[0][slot][k];
                  if(Ydaxis[ch][i-smp1]>thresh/3.)cty_plot->Fill((float)(ch+dchoffset)/2.5,(float)(i-smp1)/10.,Ydaxis[ch][i-smp1]);
                  OKd=1;
               }
             } //end ch loop
// w2
             for (Int_t ch=0;ch<240;ch++){ 
                if(w2Amax[ch]>thresh){
                  ...
                  if(Yw2axis[ch][i-smp1]>thresh/3.)ct_plot->Fill((float)(ch+w2choffset)/2.5,(float)(i-smp1)/10.,Yw2axis[ch][i-smp1]);
                  OKw2=1;
                } 
             } //end ch loop
...
void analyzeBank(evioDOMNodeP bankPtr) {
  ...
  } else if ( data_bank->tag == 76 && bankPtr->tag==17 ) { // rocTRD1  == 0x4C SRS bank = 17  SRS 
